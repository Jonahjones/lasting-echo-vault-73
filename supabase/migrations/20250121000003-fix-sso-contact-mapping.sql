-- Fix SSO User Contact Mapping and Email Normalization
-- This migration addresses all the issues found in the trusted contacts system

-- Step 1: Add missing columns to contacts table for SSO user linking
...
-- Step 5: Create improved functions for contact management
...
COMMENT ON FUNCTION public.check_contact_relationships IS 'Check if a contact exists with normalized email handling and SSO user detection';
COMMENT ON FUNCTION public.add_trusted_contact_relationship IS 'Safely add a trusted contact relationship with comprehensive validation and SSO user linking';
COMMENT ON FUNCTION public.get_user_contacts IS 'Get all contacts for a user with normalized email output';
COMMENT ON FUNCTION public.get_trusted_contact_relationships IS 'Get relationships where the specified email is the trusted contact';
COMMENT ON FUNCTION public.debug_trusted_contacts IS 'Debug function to list all trusted contacts with linking status';
COMMENT ON FUNCTION public.fix_all_contact_relationships IS 'Fix all existing contact relationships by linking to actual users'; 